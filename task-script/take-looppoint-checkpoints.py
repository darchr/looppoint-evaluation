...
for bench in benchmarks:
    run_file = Path(workdir/f"script/pre-experiment/take-looppoint-checkpoints.py")
    run_output = Path(output_dir/f"{arch}-{bench}-take-looppoint-checkpoint-m5out")
    command = [gem5_bin.as_posix(), "-re", "--outdir", run_output.as_posix(), run_file.as_posix(),
                ...
                "--output-dir", Path(workdir/f"script/workloads/checkpoints/looppoint-cpts/{arch}").as_posix()]
