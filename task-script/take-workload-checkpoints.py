...
workdir = Path().cwd()
checkpoint_output_path = Path(workdir/"script/workloads/checkpoints/workload-cpts")
...
def run(info):
    ...
    if result.returncode != 0:
        print(f"Failed to take checkpoint for {bench} {arch}")
    else:
        print(f"Successfully took checkpoint for {bench} {arch}")
    return result

all_runs = []
for bench in benchmarks:
    run_file = Path(workdir/f"script/pre-experiment/take-workload-checkpoints.py")
    run_output = Path(output_dir/f"{arch}-{bench}-B-workload-cpt-m5out")
    command = [gem5_bin.as_posix(), "-re", "--outdir", run_output.as_posix(), run_file.as_posix(),
               "--checkpoint-output-dir", checkpoint_output_path.as_posix(),
